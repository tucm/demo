...
public class Practice {
	...
	/**
	 * 问题： 在一个包含 n 个元素的无序数组 a 中，输出其最大值 max_val。 分析： 查找类问题 不是有序数组，排除二分查找，O(logN)
	 * 新增数据，都对结果有影响，所以判定时间负责度最少也要为O(n). 只需要存储最大值，不需要什么特殊的数据结构，空间复杂度为O(1)
	 */
	...
	/**
	 * 问题： 在一个数组 a = [1, 3, 4, 3, 4, 1, 3] 中，找到出现次数最多的那个数字。如果并列存在多个，随机输出一个。 分析：
	 * 查找类问题 无序数组，排除二分法，新增输入数据影响结果，最低时间复杂度O(n)即需要全部遍历
	...
	/**
	 * 问题： 给定一个整数数组 arr 和一个目标值 target，请你在该数组中找出加和等于目标值的两个整数，并返回它们在原数组中的下标 分析： 查找类问题，
	 * 最复杂时间复杂度为0（n*n），两次遍历，计算n*n种情况的值 非有序数组，排除二分查找，新增数据都会影响结果，所以最低时间复杂度为O(n)
	 * 对顺序无要求，采用哈希表 遍历一次数组，key存当前值，value存index的值
	...
	/**
	 * 问题： 给定一个字符串，逐个翻转字符串中的每个单词。例如，输入："This is a good example"，输出："example good a
	 * is This" 分析： 排序类问题 跟顺序有关，且是倒序，考虑使用栈
	 */
	...
	/**
	 * 给定一棵树，按照层次顺序遍历并打印这棵树 二叉树的遍历问题 排除：前序、中序、后序遍历 经分析，遍历优先顺序为，上比下优先，左比右优先
	 */
