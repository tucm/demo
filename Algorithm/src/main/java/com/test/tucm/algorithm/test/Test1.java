...
public class Test1 {
	 ...
	 * <p>
	 * （1）交易金额超过 1000元 （2）或者，该笔交易与另一个城市中相同客户名称的另一笔交易相隔不超过 60 分钟（含60分钟）
	 * <p>
	 * 每笔交易通过字符串 transactions[i] 标识，transactions[i] 按
	 * "{name},{time},{amount},{city}" 的格式进行记录，这些值分别表示交易的客户名称、时间（以分钟计）、金额、城市。
	 * <p>
		...
		for (String string : transactions) {
			...
			if (!map.containsKey(temp[0])) {
				...
				for (String[] s : map.get(temp[0])) {
					...
					if (temp[3].equals(s[3]))
						continue;
					String h = String.join(",", s);
					if (Math.abs(Integer.parseInt(temp[1]) - Integer.parseInt(s[1])) <= 60) {// 检查它和另一个城市中同名的另一笔交易相隔不超过
																								// 60 分钟
						if (!res.contains(h))
							res.add(h);
						flag = true;
